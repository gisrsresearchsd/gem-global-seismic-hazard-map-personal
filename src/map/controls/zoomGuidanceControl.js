...
const GUIDANCE_STATES = {
  warning: {
    message: "Zoom to level 4+ for PGA values",
    borderColor: "rgba(239, 68, 68, 0.25)",
  },

  info: {
    message: "Zoom to level 6+ for precise PGA values",
    borderColor: "rgba(245, 158, 11, 0.25)",
  },

  success: {
    message: "Optimal zoom level for PGA analysis",
...
export function createZoomGuidanceControl(map) {
  const ZoomGuidanceControl = L.Control.extend({
    ...
    onAdd() {
      const container = L.DomUtil.create("div", "zoom-guidance-control");
      ...
      const updateGuidance = () => {
        ...
        const state = getGuidanceState(zoom);

        container.textContent = state.message;

        container.style.borderColor = state.borderColor;
      };
